...
/**
 * This class holds the implemented functions called by RunBank.java in the main
 * function.
 ...
 * Part 2 - Functions used by Functions in Part 1 to help keep the code
...
public class UserInteractions {
    ...
    /**
     * When transfering money, this variable keeps track of the account index for
     * the account the user wants to transfer to.
     */
    private static int transferAccountIndex;// maybe delete?
    ...
    public static void isManager() {
        ...
        while (true) {
            ...
            // Sets isBankManager to false if user answers no.
                ...
                System.out.println("Invalid Input"); // User input is invalid
        }
    }
     ...
     * input "4" - transfers money from their account and deposits into another
     * account
    ...
    /**
     * Prompts the user to enter a monetary amount using the collectAmount method of
     * the Account.
     *
     * @param account The Account object to collect the amount for (using its
     *                collectAmount method).
    ...
    /**
     * Finds a row(customer) then creates and returns a Customer based on the
     * row(customer) found.
     ...
     */
    public static Customer createCustomer(Integer customerIDIn, String firstNameIn, String lastNameIn,
            Integer accountIDIn) {
        // Find the row based on either ID or Name
        String[] columns = findRow(customerIDIn, firstNameIn, lastNameIn, accountIDIn);

        if (columns == null)
            return null; // Return null if the row (customer) wasn't found

        Customer User = new Customer(Integer.parseInt(columns[0])); // Create Customer object

        // Create and set a Person object for the user: Person(First Name, Last Name,
        // Address)
    ...
    /**
     * Finds a row(customer) from the BankUsers.csv file based on either customerID,
     * First/Last Name, or account ID.
     * Creates an array of strings containing the contents in that row and returns
     * it.
     * 
     * @param customerIDIn
     * @param firstNameIn
     ...
     * 
     * @param functionName: Dynamic menu based on what function is wanting to be
     *                      used
